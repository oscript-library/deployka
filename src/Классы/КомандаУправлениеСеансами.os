	...
	Если мИдентификаторКластера = Неопределено Тогда
		...
		Для Каждого Стр Из МассивСтрок Цикл
			Если СтрНачинаетсяС(Стр, "cluster") Тогда
				СтруктураКластера.Вставить("УИДКластера",ЗначениеВСтрокеКластера(Стр));
			КонецЕсли;
			Если СтрНачинаетсяС(Стр, "name") Тогда
				СтруктураКластера.Вставить("ИмяКластера",ЗначениеВСтрокеКластера(Стр));
...
Функция ЗначениеВСтрокеКластера(Стр)
	Возврат СокрЛП(Сред(Стр, (СтрНайти(Стр, ": ") + 2),СтрДлина(Стр)));	
КонецФункции
