...
Функция ПолучитьСписокСоединений()
	
	СтрокаВыполнения = "%RacClient% %ХостСервера% connection %УидКластера% %АдминКластера% %ПарольАдминаКластера% list %Процесс% %УИДИБ% %АдминБазы% %ПарольАдминаБазы% > %ФайлПотока% 2> %ФайлОшибки%";
	...
	ЧислоСтрок = СтрЧислоСтрок(СписокПроцессов);
...
Функция РазобратьПоток(Знач Поток) Экспорт
	...
	Для Сч = 1 По ТД.КоличествоСтрок() Цикл
		...
		ПозицияРазделителя = Найти(Текст,":");
		Если ПозицияРазделителя = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаРазбораИмя = СокрЛП(Лев(Текст,ПозицияРазделителя-1));
		СтрокаРазбораЗначение = СокрЛП(Сред(Текст,ПозицияРазделителя+1));
