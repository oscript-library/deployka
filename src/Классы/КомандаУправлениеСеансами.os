...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Если мЭтоУправлениеСеансами Тогда
		...
		Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, 
			"-lockstarttoday",
			"Время старта блокировки пользователей сегодня, время указываем как '23:59:59' или '23:59'");
...
Процедура ПрочитатьПараметры(Знач ПараметрыКоманды)
	...
	мНастройки.Вставить("ВремяСтартаБлокировкиСегодня", ПараметрыКоманды["-lockstarttoday"]);
...
Процедура УстановитьСтатусБлокировкиСеансов(Знач Блокировать)
	...
	ИначеЕсли НЕ ПустаяСтрока(мНастройки.ВремяСтартаБлокировкиСегодня) Тогда
		ВремяСтартаБлокировкиСегодня = мНастройки.ВремяСтартаБлокировкиСегодня;
		Если СтрДлина(ВремяСтартаБлокировкиСегодня) = 5 Тогда
			ВремяСтартаБлокировкиСегодня = ВремяСтартаБлокировкиСегодня + ":00";
		КонецЕсли;

		ВремяБлокировки = Формат(ТекущаяДата(),"ДФ='yyy-MM-ddT" + ВремяСтартаБлокировкиСегодня + "'");
	КонецЕсли;
