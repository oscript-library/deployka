...
Функция ИдентификаторыКластеров()
	...
	Для А = 1 По СтрЧислоСтрок(СписокКластеров) Цикл
		лСтрока = СтрПолучитьСтроку(СписокКластеров, А);
		лИмя = СокрЛП(Лев(лСтрока, Найти(лСтрока, ":") - 1));
		лЗначение = СокрЛП(СтрЗаменить(Сред(лСтрока, Найти(лСтрока, ":") + 1), """", ""));

		Если лИмя = "cluster" Тогда
			УИДКластера = лЗначение;
		ИначеЕсли лИмя = "name" Тогда
			лСоответствие.Вставить(лЗначение, УИДКластера);
			Лог.Информация("Кластер: " + лЗначение + " = " + УИДКластера);
		КонецЕсли;
	КонецЦикла;
...
Функция НайтиБазуВКластере()
	...
	лИдентификаторыКластеров = ИдентификаторыКластеров();
	НайденаБазаВКластере = Ложь;

	Для каждого лИДКластер Из лИдентификаторыКластеров Цикл
		Лог.Информация("Поиск базы " + мНастройки.ИмяБазыДанных + " в кластере " + лИДКластер.Ключ);
		...
		Для К = 1 По ЧислоСтрок Цикл
			...
			ИначеЕсли Найти(СтрокаРазбора, "name") > 0 Тогда 
				...
				Если Нрег(ИмяБазы) = НРег(мНастройки.ИмяБазыДанных) Тогда
					мИдентификаторКластера  = лИДКластер.Значение;
					Лог.Информация("База " + ИмяБазы + " с ID: " + УИДИБ + " найдена в кластере " + лИДКластер.Ключ);
					Лог.Информация("Установлен УИД кластера " + мИдентификаторКластера);
