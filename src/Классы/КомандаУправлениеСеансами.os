...
Функция ПолучитьСписокСеансов()
    ...
    ТаблицаСеансов.Колонки.Добавить("Наименование");
	...
	Для К = 1 По ЧислоСтрок Цикл
		...
		Если СтрокаРазбораИмя = "session" Тогда
			...
			ТекСтрока.Идентификатор = СокрЛП(Сред(СтрокаРазбора,ПозицияРазделителя+1));
        ИначеЕсли СтрокаРазбораИмя = "user-name" Тогда
            ТекСтрока.Наименование = СокрЛП(Сред(СтрокаРазбора,ПозицияРазделителя+1));
		КонецЕсли;		
	КонецЦикла;
...
Процедура ОтключитьСеанс(Знач Сеанс)
    ...
    Лог.Информация("Отключаю сеанс: " + Сеанс.Наименование);
