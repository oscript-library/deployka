...
Процедура УстановитьСтатусБлокировкиСеансов(Блокировать, ВремяСтартаБлокировки="", ВремяСтартаБлокировкиЧерез="", СообщениеОблокировке="", ЧислоПопыток=1) Экспорт

	ВремяБлокировки = ВремяСтартаБлокировки;
	Если ПустаяСтрока(ВремяБлокировки) И Не ПустаяСтрока(ВремяСтартаБлокировкиЧерез) Тогда
		Секунды = 0;
		Попытка
			Секунды = Число(ВремяСтартаБлокировкиЧерез);
		Исключение
		КонецПопытки;
...
Процедура УдалитьВсеСеансыИСоединенияБазы(ЧислоПопыток=1) Экспорт
